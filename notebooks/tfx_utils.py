...
class TFXReadonlyMetadataStore(utils.ReadonlyMetadataStore):
  ...
  @staticmethod
  def from_mysql_db(host, database, user=None, password=None, port=None):
    """Returns a `TFXReadonlyMetadataStore` based off a MYSQL db config.

    Args:
      host: A `str` indicating the hostname or IP address of the MYSQL
          server.
      database: A `str` indicating the database name to connect to.
      user: (Optional) A `str` indicating the MYSQL server login id. If not set,
          the currently logged in user name is used.
      password: (Optional): A `str` indicating the password to use for `user`.
          If not set, only users with password-less login are allowed to
          connect.
      port: (Optional) An `int` indicating the TCP Port number that the MYSQL
          server accepts connections on. If not set, the default port number
          of 3306 is used.
    Returns:
      A `TFXReadonlyMetadataStore` based off a MYSQL db config.
    """
    c = metadata_store_pb2.ConnectionConfig()
    c.mysql.host = host
    c.mysql.database = database
    if user:
      c.mysql.user = user
    if password:
